...
class DefaultSettingsSpiCall extends AbstractSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest requestData) {
        ...
        try {
            ...
            Fabric.getLogger().mo4333d(Fabric.TAG, "Requesting settings from " + getUrl());
            Fabric.getLogger().mo4333d(Fabric.TAG, "Settings query params were: " + queryParams);
            toReturn = handleResponse(httpRequest);
            return toReturn;
        } finally {
            if (httpRequest != null) {
                Fabric.getLogger().mo4333d(Fabric.TAG, "Settings request ID: " + httpRequest.header(AbstractSpiCall.HEADER_REQUEST_ID));
            }
        }
    }
    ...
    JSONObject handleResponse(HttpRequest httpRequest) {
        ...
        Fabric.getLogger().mo4333d(Fabric.TAG, "Settings result was: " + statusCode);
        if (requestWasSuccessful(statusCode)) {
            return getJsonObjectFrom(httpRequest.body());
        }
        Fabric.getLogger().mo4335e(Fabric.TAG, "Failed to retrieve settings from " + getUrl());
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String httpRequestBody) {
        try {
            ...
            Fabric.getLogger().mo4334d(Fabric.TAG, "Failed to parse settings JSON from " + getUrl(), e);
            Fabric.getLogger().mo4333d(Fabric.TAG, "Settings response " + httpRequestBody);
            return null;
        }
    }
