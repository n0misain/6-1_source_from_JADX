...
class DefaultSettingsController implements SettingsController {
    ...
    public SettingsData loadSettingsData(SettingsCacheBehavior cacheBehavior) {
        ...
        try {
            ...
            Fabric.getLogger().mo4336e(Fabric.TAG, LOAD_ERROR_MESSAGE, e);
        }
        return toReturn;
    }
            ...
            if (settingsJson != null) {
                ...
                if (settingsData != null) {
                    ...
                    if (SettingsCacheBehavior.IGNORE_CACHE_EXPIRATION.equals(cacheBehavior) || !settingsData.isExpired(currentTimeMillis)) {
                        ...
                        Fabric.getLogger().mo4333d(Fabric.TAG, "Returning cached settings.");
                        return toReturn;
                    }
                    Fabric.getLogger().mo4333d(Fabric.TAG, "Cached settings have expired.");
                    return null;
                }
                Fabric.getLogger().mo4336e(Fabric.TAG, "Failed to transform cached settings data.", null);
                return null;
            }
            Fabric.getLogger().mo4333d(Fabric.TAG, "No cached settings data found.");
            return null;
        } catch (Exception e) {
            Fabric.getLogger().mo4336e(Fabric.TAG, "Failed to get cached settings", e);
            return null;
        }
    }

    private void logSettings(JSONObject json, String message) throws JSONException {
        Fabric.getLogger().mo4333d(Fabric.TAG, message + json.toString());
    }
