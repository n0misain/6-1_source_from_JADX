...
abstract class AbstractAppSpiCall extends AbstractSpiCall implements AppSpiCall {
    ...
    public boolean invoke(AppRequestData requestData) {
        ...
        Fabric.getLogger().mo4333d(Fabric.TAG, "Sending app info to " + getUrl());
        if (requestData.icon != null) {
            Fabric.getLogger().mo4333d(Fabric.TAG, "App icon hash is " + requestData.icon.hash);
            Fabric.getLogger().mo4333d(Fabric.TAG, "App icon size is " + requestData.icon.width + "x" + requestData.icon.height);
        }
        int statusCode = httpRequest.code();
        Fabric.getLogger().mo4333d(Fabric.TAG, (HttpRequest.METHOD_POST.equals(httpRequest.method()) ? "Create" : "Update") + " app request ID: " + httpRequest.header(AbstractSpiCall.HEADER_REQUEST_ID));
        Fabric.getLogger().mo4333d(Fabric.TAG, "Result was " + statusCode);
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest request, AppRequestData requestData) {
        ...
        if (requestData.icon != null) {
            ...
            try {
                ...
                Fabric.getLogger().mo4336e(Fabric.TAG, "Failed to find app icon with resource ID: " + requestData.icon.iconResourceId, e);
            } finally {
