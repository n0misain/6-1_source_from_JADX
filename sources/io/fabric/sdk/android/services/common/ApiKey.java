...
public class ApiKey {
    ...
    public static String getApiKey(Context context) {
        Fabric.getLogger().mo4346w(Fabric.TAG, "getApiKey(context) is deprecated, please upgrade kit(s) to the latest version.");
        return new ApiKey().getValue(context);
    }

    @Deprecated
    public static String getApiKey(Context context, boolean debug) {
        Fabric.getLogger().mo4346w(Fabric.TAG, "getApiKey(context, debug) is deprecated, please upgrade kit(s) to the latest version.");
        return new ApiKey().getValue(context);
    }
    ...
    protected String getApiKeyFromManifest(Context context) {
        ...
        try {
            ...
            if (bundle != null) {
                ...
                if (apiKey == null) {
                    Fabric.getLogger().mo4333d(Fabric.TAG, "Falling back to Crashlytics key lookup from Manifest");
                    apiKey = bundle.getString(CRASHLYTICS_API_KEY);
                }
            }
        } catch (Exception e) {
            Fabric.getLogger().mo4333d(Fabric.TAG, "Caught non-fatal exception while retrieving apiKey: " + e);
        }
        return apiKey;
    }
    ...
    protected String getApiKeyFromStrings(Context context) {
        ...
        if (id == 0) {
            Fabric.getLogger().mo4333d(Fabric.TAG, "Falling back to Crashlytics key lookup from Strings");
    ...
    protected void logErrorOrThrowException(Context context) {
        ...
        Fabric.getLogger().mo4335e(Fabric.TAG, buildApiKeyInstructions());
    }
