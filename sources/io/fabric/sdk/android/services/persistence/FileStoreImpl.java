...
public class FileStoreImpl implements FileStore {
    ...
    File prepare(File file) {
        if (file == null) {
            Fabric.getLogger().mo4333d(Fabric.TAG, "Null File");
        } else if (file.exists() || file.mkdirs()) {
            return file;
        } else {
            Fabric.getLogger().mo4346w(Fabric.TAG, "Couldn't create file");
        }
        return null;
    }
    ...
    boolean isExternalStorageAvailable() {
        ...
        Fabric.getLogger().mo4346w(Fabric.TAG, "External Storage is not mounted and/or writable\nHave you declared android.permission.WRITE_EXTERNAL_STORAGE in the manifest?");
        return false;
    }
}
