...
class FabricKitsFinder implements Callable<Map<String, KitInfo>> {
    ...
    public Map<String, KitInfo> call() throws Exception {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (entry.getName().startsWith(FABRIC_DIR) && entry.getName().length() > FABRIC_DIR.length()) {
                ...
                if (kitInfo != null) {
                    ...
                    Fabric.getLogger().mo4344v(Fabric.TAG, String.format("Found kit:[%s] version:[%s]", new Object[]{kitInfo.getIdentifier(), kitInfo.getVersion()}));
                }
            }
        }
        if (apkFile != null) {
        ...
        Fabric.getLogger().mo4344v(Fabric.TAG, "finish scanning in " + (SystemClock.elapsedRealtime() - startScan) + " reading:" + count);
        return kitInfos;
    }
    ...
    private KitInfo loadKitInfo(ZipEntry fabricFile, ZipFile apk) {
        ...
        try {
            ...
            kitInfo.mo4336e(Fabric.TAG, "Error when parsing fabric properties " + fabricFile.getName(), ie);
            return null;
        } finally {
