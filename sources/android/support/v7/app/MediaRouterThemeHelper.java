...
final class MediaRouterThemeHelper {
    ...
    public static Context createThemedContext(Context context, int style) {
        ...
        if (isLightTheme(context)) {
            if (getControllerColor(context, style) == COLOR_DARK_ON_LIGHT_BACKGROUND) {
                theme = C0318R.style.Theme_MediaRouter_Light;
            } else {
                theme = C0318R.style.Theme_MediaRouter_Light_DarkControlPanel;
            }
        } else if (getControllerColor(context, style) == COLOR_DARK_ON_LIGHT_BACKGROUND) {
            theme = C0318R.style.Theme_MediaRouter_LightControlPanel;
        } else {
            theme = C0318R.style.Theme_MediaRouter;
        }
        int mediaRouteThemeResId = getThemeResource(context, C0318R.attr.mediaRouteTheme);
    ...
    public static int getControllerColor(Context context, int style) {
        if (ColorUtils.calculateContrast(-1, getThemeColor(context, style, C0302R.attr.colorPrimary)) >= 3.0d) {
            return -1;
        }
    ...
    public static int getButtonTextColor(Context context) {
        int primaryColor = getThemeColor(context, 0, C0302R.attr.colorPrimary);
        if (ColorUtils.calculateContrast(primaryColor, getThemeColor(context, 0, 16842801)) < 3.0d) {
            return getThemeColor(context, 0, C0302R.attr.colorAccent);
        }
        return primaryColor;
    }
    ...
    public static void setMediaControlsBackgroundColor(Context context, View mainControls, View groupControls, boolean hasGroup) {
        int primaryColor = getThemeColor(context, 0, C0302R.attr.colorPrimary);
        int primaryDarkColor = getThemeColor(context, 0, C0302R.attr.colorPrimaryDark);
    ...
    public static int getAlertDialogResolvedTheme(Context context, int themeResId) {
        ...
        context.getTheme().resolveAttribute(C0302R.attr.alertDialogTheme, outValue, true);
        return outValue.resourceId;
    }

    private static boolean isLightTheme(Context context) {
        TypedValue value = new TypedValue();
        if (!context.getTheme().resolveAttribute(C0302R.attr.isLightTheme, value, true) || value.data == 0) {
            return false;
        }
        return true;
    }
