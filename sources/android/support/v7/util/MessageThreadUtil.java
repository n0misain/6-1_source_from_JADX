...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C03251();
            ...
            class C03251 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C03261.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            class C03271 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C03282.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C03282.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C03282.this.mQueue.removeMessages(2);
                                    C03282.this.mQueue.removeMessages(3);
                        ...
                        C03282.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
