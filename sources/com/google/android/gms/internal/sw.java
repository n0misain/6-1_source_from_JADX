...
final class sw implements Callable<List<? extends vk>> {
    ...
    public final /* synthetic */ Object call() throws Exception {
        ...
        uv zzHR = uv.zzHR();
        if (zzav.zzHw()) {
            zzb = zzHR.zzb(qr.zzGZ(), Boolean.valueOf(true));
        } else {
            Iterator it = zzav.zzHv().iterator();
            zzb = zzHR;
            while (it.hasNext()) {
                zzb = zzb.zzb((qr) ((Entry) it.next()).getKey(), Boolean.valueOf(true));
            }
        }
        return this.zzceR.zza(new tu(zzav.zzFq(), zzb, this.zzcfb));
    }
}
