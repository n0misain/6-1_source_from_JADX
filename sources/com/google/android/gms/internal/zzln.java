package com.google.android.gms.internal;

import com.google.android.gms.dynamic.IObjectWrapper;

public final class zzln extends zzka {
    private zzjo zztK;

    public final void destroy() {
    }

    public final String getAdUnitId() {
        return null;
    }

    public final String getMediationAdapterClassName() {
        return null;
    }

    public final zzks getVideoController() {
        return null;
    }

    public final boolean isLoading() {
        return false;
    }

    public final boolean isReady() {
        return false;
    }

    public final void pause() {
    }

    public final void resume() {
    }

    public final void setImmersiveMode(boolean z) {
    }

    public final void setManualImpressionsEnabled(boolean z) {
    }

    public final void setUserId(String str) {
    }

    public final void showInterstitial() {
    }

    public final void stopLoading() {
    }

    public final void zza(zzadd zzadd) {
    }

    public final void zza(zziv zziv) {
    }

    public final void zza(zzjl zzjl) {
    }

    public final void zza(zzjo zzjo) {
        this.zztK = zzjo;
    }

    public final void zza(zzke zzke) {
    }

    public final void zza(zzkk zzkk) {
    }

    public final void zza(zzky zzky) {
    }

    public final void zza(zzlx zzlx) {
    }

    public final void zza(zznh zznh) {
    }

    public final void zza(zzxg zzxg) {
    }

    public final void zza(zzxo zzxo, String str) {
    }

    public final boolean zza(zzir zzir) {
        zzajc.m1216e("This app is using a lightweight version of the Google Mobile Ads SDK that requires the latest Google Play services to be installed, but Google Play services is either missing or out of date.");
        zzaiy.zzaaH.post(new zzlo(this));
        return false;
    }

    public final String zzaI() {
        return null;
    }

    public final IObjectWrapper zzal() {
        return null;
    }

    public final zziv zzam() {
        return null;
    }

    public final void zzao() {
    }

    public final zzke zzax() {
        return null;
    }

    public final zzjo zzay() {
        return null;
    }
}
