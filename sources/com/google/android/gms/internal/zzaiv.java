...
final class zzaiv implements Callable<String> {
    ...
    public final /* synthetic */ Object call() throws Exception {
        ...
        int i = 0;
        if (this.zzaay != null) {
            zzafr.m1217v("Attempting to read user agent from Google Play Services.");
            sharedPreferences = this.zzaay.getSharedPreferences("admob_user_agent", 0);
        } else {
            zzafr.m1217v("Attempting to read user agent from local cache.");
        ...
        if (TextUtils.isEmpty(string)) {
            zzafr.m1217v("Reading user agent from WebSettings");
            string = WebSettings.getDefaultUserAgent(this.zztF);
            if (i != 0) {
                sharedPreferences.edit().putString("user_agent", string).apply();
                zzafr.m1217v("Persisting user agent.");
            }
        }
        return string;
    }
