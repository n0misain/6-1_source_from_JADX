...
public final class zzac extends zzajt<zza> {
    ...
    private final void zzfd() {
        synchronized (this.mLock) {
            ...
            if (this.zzLu && this.zzLv == 0) {
                zzafr.m1217v("No reference is left (including root). Cleaning up engine.");
                zza(new zzaf(this), new zzajr());
            } else {
                zzafr.m1217v("There are still references to the engine. Not destroying.");
            }
        }
    }
    ...
    protected final void zzfb() {
        synchronized (this.mLock) {
            zzbo.zzae(this.zzLv > 0);
            zzafr.m1217v("Releasing 1 reference for JS Engine");
            this.zzLv--;
            zzfd();
        }
    }
    ...
    public final void zzfc() {
        ...
        synchronized (this.mLock) {
            ...
            zzbo.zzae(z);
            zzafr.m1217v("Releasing root reference. JS Engine will be destroyed once other references are released.");
            this.zzLu = true;
            zzfd();
        }
    }
