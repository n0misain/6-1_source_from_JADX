...
final class DecodedBitStreamParser {
    ...
    static DecoderResult decode(byte[] bytes, int mode) {
        StringBuilder result = new StringBuilder(Errors.ALREADY_UNFAVORITED);
        switch (mode) {
            case 2:
            case 3:
                String postcode;
        ...
        while (i < start + len) {
            ...
            switch (c) {
                ...
                case '￻':
                    ...
                    i++;
                    i++;
                    sb.append(new DecimalFormat("000000000").format((long) (((((bytes[i] << 24) + (bytes[i] << 18)) + (bytes[i] << 12)) + (bytes[i] << 6)) + bytes[i])));
                    shift2 = shift;
                    break;
                default:
                    sb.append(c);
