    ...
    static int lookAheadTest(CharSequence msg, int startpos, int currentMode) {
        ...
        while (startpos + charsProcessed != msg.length()) {
            ...
            if (charsProcessed >= 4) {
                ...
                mins = new byte[6];
                findMinimums(charCounts, intCharCounts, ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED, mins);
        ...
        mins = new byte[6];
        intCharCounts = new int[6];
        int min = findMinimums(charCounts, intCharCounts, ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED, mins);
        minCount = getMinimumCount(mins);
