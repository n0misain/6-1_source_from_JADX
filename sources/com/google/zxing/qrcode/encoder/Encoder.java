...
public final class Encoder {
    ...
    public static QRCode encode(String content, ErrorCorrectionLevel ecLevel, Map<EncodeHintType, ?> hints) throws WriterException {
        ...
        if (hints != null) {
            if (hints.containsKey(EncodeHintType.QR_VERSION)) {
                ...
                finalBits = interleaveWithECBytes(headerAndDataBits, version.getTotalCodewords(), numDataBytes, ecBlocks.getNumBlocks());
                qrCode = new QRCode();
    ...
    private static boolean isOnlyDoubleByteKanji(String content) {
        try {
            ...
            for (int i = 0; i < length; i += 2) {
                ...
                if ((byte1 < 129 || byte1 > 159) && (byte1 < 224 || byte1 > TwitterApiErrorConstants.EXPIRED_LOGIN_VERIFICATION_REQUEST)) {
                    return false;
                }
            }
            return true;
    ...
    static void terminateBits(int numDataBytes, BitArray bits) throws WriterException {
        ...
        for (i = 0; i < numPaddingBytes; i++) {
            bits.appendBits((i & 1) == 0 ? TwitterApiErrorConstants.LOGIN_INCORRECT_CHALLENGE_RESPONSE : 17, 8);
        }
        if (bits.getSize() != capacity) {
