...
public class LoginButton extends FacebookButtonBase {
    ...
    protected class LoginClickListener implements OnClickListener {
        ...
        protected void performLogout(Context context) {
            ...
            if (LoginButton.this.confirmLogout) {
                String message;
                String logout = LoginButton.this.getResources().getString(C0665R.string.com_facebook_loginview_log_out_action);
                String cancel = LoginButton.this.getResources().getString(C0665R.string.com_facebook_loginview_cancel_action);
                Profile profile = Profile.getCurrentProfile();
                if (profile == null || profile.getName() == null) {
                    message = LoginButton.this.getResources().getString(C0665R.string.com_facebook_loginview_logged_in_using_facebook);
                } else {
                    message = String.format(LoginButton.this.getResources().getString(C0665R.string.com_facebook_loginview_logged_in_as), new Object[]{profile.getName()});
                }
    ...
    class C07602 extends AccessTokenTracker {
        C07602() {
        }
    ...
    private void checkToolTipSettings() {
        switch (this.toolTipMode) {
            case AUTOMATIC:
                ...
                return;
            case DISPLAY_ALWAYS:
                displayToolTip(getResources().getString(C0665R.string.com_facebook_tooltip_default));
                return;
            default:
                return;
        }
    ...
    protected void configureButton(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (isInEditMode()) {
            setBackgroundColor(getResources().getColor(C0665R.color.com_facebook_blue));
            this.loginText = "Continue with Facebook";
        } else {
            this.accessTokenTracker = new C07602();
        }
        setButtonText();
        setCompoundDrawablesWithIntrinsicBounds(AppCompatResources.getDrawable(getContext(), C0665R.drawable.com_facebook_button_login_logo), null, null, null);
    }
    ...
    protected int getDefaultStyleResource() {
        return C0665R.style.com_facebook_loginview_default_style;
    }

    private void parseLoginButtonAttributes(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        this.toolTipMode = ToolTipMode.DEFAULT;
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, C0665R.styleable.com_facebook_login_view, defStyleAttr, defStyleRes);
        try {
            this.confirmLogout = a.getBoolean(C0665R.styleable.com_facebook_login_view_com_facebook_confirm_logout, true);
            this.loginText = a.getString(C0665R.styleable.com_facebook_login_view_com_facebook_login_text);
            this.logoutText = a.getString(C0665R.styleable.com_facebook_login_view_com_facebook_logout_text);
            this.toolTipMode = ToolTipMode.fromInt(a.getInt(C0665R.styleable.com_facebook_login_view_com_facebook_tooltip_mode, ToolTipMode.DEFAULT.getValue()));
        } finally {
            a.recycle();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (text == null) {
            text = resources.getString(C0665R.string.com_facebook_loginview_log_in_button_continue);
            logInWidth = measureButtonWidth(text);
            if (resolveSize(logInWidth, widthMeasureSpec) < logInWidth) {
                text = resources.getString(C0665R.string.com_facebook_loginview_log_in_button);
            }
        }
        logInWidth = measureButtonWidth(text);
        text = this.logoutText;
        if (text == null) {
            text = resources.getString(C0665R.string.com_facebook_loginview_log_out_button);
        }
    ...
    private void setButtonText() {
        ...
        if (!isInEditMode() && AccessToken.getCurrentAccessToken() != null) {
            ...
            if (this.logoutText != null) {
                ...
                charSequence = resources.getString(C0665R.string.com_facebook_loginview_log_out_button);
            }
            setText(charSequence);
        } else if (this.loginText != null) {
            setText(this.loginText);
        } else {
            String text = resources.getString(C0665R.string.com_facebook_loginview_log_in_button_continue);
            int width = getWidth();
