...
public class ActivityLifecycleTracker {
    ...
    static class C06841 implements ActivityLifecycleCallbacks {
        C06841() {
        }
    ...
    public static void startTracking(Application application, String appId) {
        if (tracking.compareAndSet(false, true)) {
            appId = appId;
            application.registerActivityLifecycleCallbacks(new C06841());
        }
    }
    ...
    private static void onActivityPaused(Activity activity) {
        ...
        singleThreadExecutor.execute(new Runnable() {
            ...
            public void run() {
                ...
                ActivityLifecycleTracker.currentSession.setSessionLastEventTime(Long.valueOf(currentTime));
                if (ActivityLifecycleTracker.foregroundActivityCount.get() <= 0) {
                    ActivityLifecycleTracker.currentFuture = ActivityLifecycleTracker.singleThreadExecutor.schedule(new C06871(), (long) ActivityLifecycleTracker.getSessionTimeoutInSeconds(), TimeUnit.SECONDS);
                }
