...
final class ScribeConstants {
    ...
    static EventNamespace getSyndicatedSdkTimelineNamespace(String timelineType) {
        return new Builder().setClient("android").setPage("timeline").setSection(timelineType).setComponent("initial").setElement("").setAction(SCRIBE_IMPRESSION_ACTION).builder();
    }

    static EventNamespace getTfwClientTimelineNamespace(String timelineType) {
        return new Builder().setClient(SyndicationClientEvent.CLIENT_NAME).setPage("android").setSection("timeline").setComponent(timelineType).setElement("initial").setAction(SCRIBE_IMPRESSION_ACTION).builder();
    }

    static EventNamespace getTfwClientFilterTimelineNamespace(String timelineType) {
        return new Builder().setClient(SyndicationClientEvent.CLIENT_NAME).setPage("android").setSection("timeline").setComponent(timelineType).setElement("initial").setAction(SCRIBE_FILTER_ACTION).builder();
    }
}
