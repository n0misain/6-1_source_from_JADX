...
class SSOAuthHandler extends AuthHandler {
    ...
    private boolean startAuthActivityForResult(Activity activity) {
        String packageName = availableSSOPackage(activity.getPackageManager());
        if (packageName == null) {
            Fabric.getLogger().mo4336e(TwitterCore.TAG, "SSO app signature check failed", null);
            return false;
        }
        ...
        if (IntentUtils.isActivityAvailable(activity, intent)) {
            ...
            try {
                ...
                Fabric.getLogger().mo4336e(TwitterCore.TAG, "SSO exception occurred", e);
                return false;
            }
        }
        Fabric.getLogger().mo4336e(TwitterCore.TAG, "SSO auth activity not found", null);
        return false;
    }
    ...
    private static boolean checkAppSignature(PackageManager pm, String packageName, String requiredSignature) {
        try {
            for (Signature s : pm.getPackageInfo(packageName, 64).signatures) {
