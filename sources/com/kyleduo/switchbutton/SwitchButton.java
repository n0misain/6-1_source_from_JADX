...
public class SwitchButton extends CompoundButton {
    ...
    static class SavedState extends BaseSavedState {
        public static final Creator<SavedState> CREATOR = new C09641();
        CharSequence offText;
        CharSequence onText;

        /* renamed from: com.kyleduo.switchbutton.SwitchButton$SavedState$1 */
        static class C09641 implements Creator<SavedState> {
            C09641() {
            }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs == null) {
            ...
            ta = getContext().obtainStyledAttributes(attrs, C0962R.styleable.SwitchButton);
        }
        if (ta != null) {
            thumbDrawable = ta.getDrawable(C0962R.styleable.SwitchButton_kswThumbDrawable);
            thumbColor = ta.getColorStateList(C0962R.styleable.SwitchButton_kswThumbColor);
            margin = ta.getDimension(C0962R.styleable.SwitchButton_kswThumbMargin, margin);
            marginLeft = ta.getDimension(C0962R.styleable.SwitchButton_kswThumbMarginLeft, margin);
            marginRight = ta.getDimension(C0962R.styleable.SwitchButton_kswThumbMarginRight, margin);
            marginTop = ta.getDimension(C0962R.styleable.SwitchButton_kswThumbMarginTop, margin);
            marginBottom = ta.getDimension(C0962R.styleable.SwitchButton_kswThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(C0962R.styleable.SwitchButton_kswThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(C0962R.styleable.SwitchButton_kswThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(C0962R.styleable.SwitchButton_kswThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.0f);
            backRadius = ta.getDimension(C0962R.styleable.SwitchButton_kswBackRadius, (2.0f * density) + thumbRadius);
            backDrawable = ta.getDrawable(C0962R.styleable.SwitchButton_kswBackDrawable);
            backColor = ta.getColorStateList(C0962R.styleable.SwitchButton_kswBackColor);
            backMeasureRatio = ta.getFloat(C0962R.styleable.SwitchButton_kswBackMeasureRatio, 1.8f);
            animationDuration = ta.getInteger(C0962R.styleable.SwitchButton_kswAnimationDuration, 250);
            fadeBack = ta.getBoolean(C0962R.styleable.SwitchButton_kswFadeBack, true);
            tintColor = ta.getColor(C0962R.styleable.SwitchButton_kswTintColor, 0);
            textOn = ta.getString(C0962R.styleable.SwitchButton_kswTextOn);
            textOff = ta.getString(C0962R.styleable.SwitchButton_kswTextOff);
            textMarginH = ta.getDimension(C0962R.styleable.SwitchButton_kswTextMarginH, Math.max(textMarginH, backRadius / 2.0f));
            autoAdjustTextPosition = ta.getBoolean(C0962R.styleable.SwitchButton_kswAutoAdjustTextPosition, true);
            ta.recycle();
        }
        if (attrs == null) {
            ta = null;
            ...
            int[] iArr = new int[2];
            ta = getContext().obtainStyledAttributes(attrs, new int[]{16842970, 16842981});
        }
        if (ta != null) {
        ...
        this.mAutoAdjustTextPosition = autoAdjustTextPosition;
        ...
        if (this.mTintColor == 0) {
            ...
            if (getContext().getTheme().resolveAttribute(C0962R.attr.colorAccent, typedValue, true)) {
    ...
    private Layout makeLayout(CharSequence text) {
        return new StaticLayout(text, this.mTextPaint, (int) Math.ceil((double) Layout.getDesiredWidth(text, this.mTextPaint)), Alignment.ALIGN_CENTER, 1.0f, 0.0f, false);
    }
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        int minWidth = ceil((double) (this.mThumbSizeF.x * this.mBackMeasureRatio));
        ...
        minWidth = Math.max(minWidth, ceil((double) ((((float) minWidth) + this.mThumbMargin.left) + this.mThumbMargin.right)));
        minWidth = Math.max(Math.max(minWidth, (getPaddingLeft() + minWidth) + getPaddingRight()), getSuggestedMinimumWidth());
        if (widthMode == 1073741824) {
            return Math.max(minWidth, widthSize);
        }
        int measuredWidth = minWidth;
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        int minHeight = ceil((double) Math.max(this.mThumbSizeF.y, (this.mThumbSizeF.y + this.mThumbMargin.top) + this.mThumbMargin.right));
        if (this.mOnLayout != null) {
        ...
        if (onHeight == 0.0f && offHeight == 0.0f) {
            ...
            minHeight = ceil((double) Math.max((float) minHeight, this.mTextHeight));
        }
        minHeight = Math.max(minHeight, getSuggestedMinimumHeight());
        minHeight = Math.max(minHeight, (getPaddingTop() + minHeight) + getPaddingBottom());
        if (heightMode == 1073741824) {
            return Math.max(minHeight, heightSize);
        }
        int measuredHeight = minHeight;
    ...
    private void setup() {
        ...
        if (this.mOnLayout != null) {
            float marginOnX = (((((this.mBackRectF.width() - this.mThumbRectF.width()) - this.mThumbMargin.right) - ((float) this.mOnLayout.getWidth())) / 2.0f) + this.mBackRectF.left) + (this.mThumbMargin.left < 0.0f ? this.mThumbMargin.left * -0.5f : 0.0f);
            if (!this.mIsBackUseDrawable && this.mAutoAdjustTextPosition) {
                marginOnX += this.mBackRadius / 4.0f;
            }
        ...
        if (this.mOffLayout != null) {
            float marginOffX = ((this.mBackRectF.right - ((((this.mBackRectF.width() - this.mThumbRectF.width()) - this.mThumbMargin.left) - ((float) this.mOffLayout.getWidth())) / 2.0f)) - ((float) this.mOffLayout.getWidth())) + (this.mThumbMargin.right < 0.0f ? this.mThumbMargin.right * 0.5f : 0.0f);
            if (!this.mIsBackUseDrawable && this.mAutoAdjustTextPosition) {
                marginOffX -= this.mBackRadius / 4.0f;
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mIsBackUseDrawable) {
            ...
            alpha = (int) (255.0f * getProcess());
            this.mPaint.setARGB((Color.alpha(belowColor) * alpha) / 255, Color.red(belowColor), Color.green(belowColor), Color.blue(belowColor));
            canvas.drawRoundRect(this.mBackRectF, this.mBackRadius, this.mBackRadius, this.mPaint);
            this.mPaint.setARGB((Color.alpha(aboveColor) * (255 - alpha)) / 255, Color.red(aboveColor), Color.green(aboveColor), Color.blue(aboveColor));
            canvas.drawRoundRect(this.mBackRectF, this.mBackRadius, this.mBackRadius, this.mPaint);
        ...
        if (!(switchText == null || textRectF == null)) {
            int textColor;
            float process = ((double) getProcess()) >= 0.75d ? (getProcess() * 4.0f) - 3.0f : ((double) getProcess()) < 0.25d ? 1.0f - (getProcess() * 4.0f) : 0.0f;
            alpha = (int) (process * 255.0f);
            if (((double) getProcess()) > 0.5d) {
                textColor = this.mOnTextColor;
            } else {
                textColor = this.mOffTextColor;
            }
            switchText.getPaint().setARGB((Color.alpha(textColor) * alpha) / 255, Color.red(textColor), Color.green(textColor), Color.blue(textColor));
            canvas.save();
